...
namespace alarmbot.Log
{
    public class Logs : ILazy<Logs>
    {
        ...
        private void write_log(DateTime dt, string message)
        {
            ...
            lock (log_lock)
            {
                File.AppendAllText(Path.Combine(Program.ApplicationPath, "log.txt"), $"[{dt.ToString(en)}] {message}\r\n");
            }
        }
        ...
        private void write_error_log(DateTime dt, string message)
        {
            ...
            lock (log_lock)
            {
                File.AppendAllText(Path.Combine(Program.ApplicationPath, "log.txt"), $"[{dt.ToString(en)}] [Error] {message}\r\n");
            }
        }
    }
}
